 ...
 */
export const MODEL_FAMILY_ALIASES = ['sonnet', 'opus', 'haiku', 'chat', 'reasoner'] as const

export function isModelFamilyAlias(model: string): boolean {
  return (MODEL_FAMILY_ALIASES as readonly string[]).includes(model)
}

/** Resolve a DeepSeek alias to a model ID */
export function resolveDeepSeekAlias(alias: string): string | null {
  switch (alias) {
    case 'chat':
    case 'fast':
    case 'v3':
